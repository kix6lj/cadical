...
bool Internal::propagate() {
  ...
  while (!conflict) {
    ...
    vector<int> updated(trail.begin() + last_pos, trail.end());
    auto update_lits = xclauses->propagate(updated, level);

    if (xclauses->conflict_flag == true) {
      conflict = xclauses->conflict;
      break;
    }
    
    if (update_lits.size() == 0)
      break;
...
#endif
      xor_deduce_assign(lit.first, get_gaussian_block(vidx(lit.first)), lit.second);
    }
