...
struct Internal {
  ...
  vector<int> gaussian_pos;    // position of each variable in its
  vector<int> gaussian_block;  // corresponding gaussian equation block
  ...
  void push_literals_of_block(const std::vector<int>::reverse_iterator &,
                              const std::vector<int>::reverse_iterator &, int,
                              unsigned);
  ...
  unsigned shrink_block(std::vector<int>::reverse_iterator &,
                        std::vector<int>::reverse_iterator &, int, unsigned &,
                        unsigned &, const int, unsigned);
  ...
  void xor_deduce_assign(int lit, int block, int eqn);
  ...
  // Utilities for gaussian elimination, implemented in gaussian.cpp
  //
  inline int get_gaussian_pos(int var) const { return gaussian_pos[var]; }

  inline void set_gaussian_pos(int var, int pos) { gaussian_pos[var] = pos; }

  inline int get_gaussian_block(int var) const { return gaussian_block[var]; }

  inline void set_gaussian_block(int var, int block) {
    gaussian_block[var] = block;
  }
  ...
  void verbose(int level, const char *fmt, ...) CADICAL_ATTRIBUTE_FORMAT(3, 4);
